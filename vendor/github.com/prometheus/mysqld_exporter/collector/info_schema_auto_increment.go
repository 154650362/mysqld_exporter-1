...
var (
	globalInfoSchemaAutoIncrementDesc = prometheus.NewDesc(
		...
		[]string{"schema", "table", "column", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapeAutoIncrementColumns) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for autoIncrementRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			schema, table, column, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
