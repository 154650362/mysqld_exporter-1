...
var (
	performanceSchemaEventsStatementsDesc = prometheus.NewDesc(
		...
		[]string{"schema", "digest", "digest_text", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapePerfEventsStatements) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for perfSchemaEventsStatementsRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			schemaName, digest, digestText, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
