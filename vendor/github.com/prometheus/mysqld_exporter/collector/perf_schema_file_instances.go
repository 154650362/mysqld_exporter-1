...
var (
	...
	performanceSchemaFileInstancesBytesDesc = prometheus.NewDesc(
		...
		[]string{"file_name", "event_name", "mode", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapePerfFileInstances) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for perfSchemaFileInstancesRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			fileName, eventName, "read", Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
			performanceSchemaFileInstancesCountDesc, prometheus.CounterValue, float64(countWrite),
			fileName, eventName, "write", Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
