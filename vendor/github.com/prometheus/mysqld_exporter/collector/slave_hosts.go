...
var (
	SlaveHostsInfo = prometheus.NewDesc(
		...
		[]string{"server_id", "slave_host", "port", "master_id", "slave_uuid", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
)
...
func (ScrapeSlaveHosts) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
