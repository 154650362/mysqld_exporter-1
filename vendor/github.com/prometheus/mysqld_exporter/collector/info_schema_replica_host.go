...
var (
	infoSchemaReplicaHostCpuDesc = prometheus.NewDesc(
		...
		[]string{"server_id", "role", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapeReplicaHost) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for replicaHostRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			serverId, role, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
