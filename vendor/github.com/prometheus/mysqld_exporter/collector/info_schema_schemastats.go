...
var (
	infoSchemaStatsRowsReadDesc = prometheus.NewDesc(
		...
		[]string{"schema", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapeSchemaStat) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for informationSchemaTableStatisticsRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			tableSchema, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
