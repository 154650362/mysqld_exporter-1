...
var (
	performanceSchemaEventsWaitsDesc = prometheus.NewDesc(
		...
		[]string{"event_name", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapePerfEventsWaits) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for perfSchemaEventsWaitsRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			eventName, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
