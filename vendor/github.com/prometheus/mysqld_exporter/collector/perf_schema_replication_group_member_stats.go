...
var (
	performanceSchemaReplicationGroupMemberStatsTransInQueueDesc = prometheus.NewDesc(
		...
		[]string{"member_id", "_workgroup_name", "workgroup_name", "addr", "cluster", "instanceId", "workgroup_id"}, nil,
	)
...
func (ScrapePerfReplicationGroupMemberStats) Scrape(ctx context.Context, db *sql.DB, ch chan<- prometheus.Metric, logger log.Logger, Instance master.Instance) error {
	...
	for perfReplicationGroupMemeberStatsRows.Next() {
		...
		ch <- prometheus.MustNewConstMetric(
			...
			memberId, Instance.WorkGroupName, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupName, Instance.Addr, Instance.WorkGroupID,
		)
		ch <- prometheus.MustNewConstMetric(
